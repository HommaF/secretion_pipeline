...
tail -n +3 $signalp | sort -k1,1 | sed 's/\ /_/g' | sed 's/pos\:\t/pos\:_/g' | awk '{if ($5 ~ "CS") print; else print $1 "\t" $2 "\t" $3 "\t" $4 "\tNaN"}' > output/$sample/tmp_signalp.tsv
tail -n +3 $targetp | sort -k1,1 | cut -f2- | sed 's/\ /_/g' | sed 's/pos\:\t/pos\:_/g' | awk '{if ($7 ~ "CS") print; else print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $6 "\tNan"}' > output/$sample/tmp_targetp.tsv
